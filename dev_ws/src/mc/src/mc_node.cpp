...
unsigned char g_mc_8007[] = {237,222,11,4,7,128,00,97,02,13,10};
...
public:
    TopicSuscribe01(std::string name) : Node(name)
    {
        ...
        control_subscriber_ = this->create_subscription<message_interfaces::msg::Control>("control_command",10,std::bind(&TopicSuscribe01::control_command_callback,this,std::placeholders::_1));
...
private:
    rclcpp::Subscription<message_interfaces::msg::Control>::SharedPtr control_subscriber_;
    ...
    void control_command_callback(const message_interfaces::msg::Control::SharedPtr msg)
    {

      RCLCPP_INFO(this->get_logger(),"\n/control_command收到的\nV_cx=[%s]\tW_z=[%s]\n/control_command数据接收OK\n",msg->v_c.c_str(),msg->w_c.c_str());
...
void* thread_uasrtsend(void* args){
    ...
    if (g_usart_fd >= 0) {
        ...
        sleep(0.5);
        ntzx_usart_send(g_usart_fd,(char*)g_mc_8007,12);
    }
    else{
    ...
    //初始化地盘数据
      ...
      V_r_high = V_r / 256;
      //四轮速度
      g_mc_ctrl_string[7] = g_mc_ctrl_string[11] = V_r_low;
      g_mc_ctrl_string[8] = g_mc_ctrl_string[12] = V_r_high;
      g_mc_ctrl_string[9] = g_mc_ctrl_string[13] = V_l_low;
      g_mc_ctrl_string[10] = g_mc_ctrl_string[14] = V_l_high;

      //计算速度校验位
      int sum;
      sum = 622+(V_r_low+V_r_high)*2+(V_l_low+V_l_high)*2;
      ...
      int rt;
      rt = ntzx_usart_send(g_usart_fd,(char*)g_mc_ctrl_string, 24);
      // cout<<"rt="<<rt<<endl;
...
int main(int argc, char **argv)
{
    ...
    int ret;
    //创建串口接收、发送线程
    ret = pthread_create( &id1, NULL, thread_uasrtsend, NULL );
    ret = pthread_create( &id2, NULL, thread_uasrtrecv, NULL );
