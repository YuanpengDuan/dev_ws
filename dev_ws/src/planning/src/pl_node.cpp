   ...
   void timer_callback()
    {
        ...
        message.right_low = to_string(speed_write.right_low);
...
{   
    ...
    float angle_Dest = g_road_info_pl->waypoint_info[dest_index].courseAngle;
    //将修正角度始终控制在+/-180度。
    ...
    // mc_Param.courseAngle = (int)(atan(point_x / point_y) *RAD2DEG)+(int)(ki_fixed*angle_Compensation); //单位是角度
       mc_Param.courseAngle = (int)(atan(point_x / point_y))+(int)(ki_fixed*angle_Compensation); //单位是角度
        ...
        for ( int i = (nearest_point_index+CAR_FRONT_OFFET); i < (nearest_point_index+CAR_FRONT_OFFET+n_point_need_to_detect); i++)
        {
            ...
            {   
                ...
                for (int  column = 0; column < 12; column++)
                {
                    ...
                    // Car_Occupy_temp[row][column].x =  Car_Occupy_temp[row][column].x+(x_diff);
                    // Car_Occupy_temp[row][column].y =  Car_Occupy_temp[row][column].y+(y_diff);
                    // printf("[%d,%d]\t",Car_Occupy_temp[row][column].x,Car_Occupy_temp[row][column].y);
                    //将雷达位置转换成数组坐标
